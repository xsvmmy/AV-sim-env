...
class TrainingEntry(BaseModel):
    response_id: str
    choice: str = Field(..., pattern="^(stay|swerve)$")


class TrainCustomAgentRequest(BaseModel):
    name: str = Field(..., min_length=1, max_length=40)
    training_data: List[TrainingEntry] = Field(..., min_length=1)
...
class CustomAgentPredictRequest(BaseModel):
    passengers: List[str]
    pedestrians: List[str]
    traffic_light: str = Field(..., pattern="^(Red|Green)$")


class CustomAgentPredictResponse(BaseModel):
    action: str
    voting_method: str
    credences: dict
